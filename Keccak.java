...
{
	public static int w;
	//Secure coding rule: MSC55-J followed
	...
	static String ConvertStringToBinary(String s)
	{
		...
		try 
		{
			...
			binary=new BigInteger(s.getBytes("UTF-16")).toString(2);
			return binary;
		} 
	...
	//Converts a given string of 0's and 1's to a string of English language
	...
	//Convert the 3D state to a 1D state BitSet 
	...
	//Convert the 1D state to a 3D state BitSet
	...
	//Pad generation function for Keccak
	...
	static BitSet[][] IotaStep(BitSet[][] A, int iR)
	{
		...
		BitSet RC= new BitSet(w);
		Int l=0;
		//Secure coding rule: NUM02-J followed
		l=(int)Math.floor(Math.log(w)/Math.log(2));
	...
	//SPONGE function for SHA3	
	static String SPONGE(String N,int d)
	{
	  int r=576, b=1600;	
	  String P=N+KeccakPad(r,N.length());   
